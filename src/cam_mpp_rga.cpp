...
MPP_RET test_mpp_run(MpiEncMultiCtxInfo *info)
{
    ...
    // std::cout << "mpp_frame_set_eos :  " <<   p->height << std::endl;
    ...
    while (!p->pkt_eos) {
        double ttt = what_time_is_it_now();
        MppMeta meta = NULL;
        MppFrame frame = NULL;
        MppPacket packet = NULL;
        //获得开辟的内存的首地址
        ...
        ret = mpp_frame_init(&frame);  //为什么初始化成功？？ frame一直是null，在初始化的时候为空会报错？有关于null与空指针含义。
        if (ret) {
        ...
        std::cout << "----mpp_frame_set_fmt---- :  " <<   p->fmt << std::endl;
        ...
        if (p->fp_input && feof(p->fp_input)){
            ...
            std::cout << "*******************" << std::endl;
            base = (RK_U8 *)mpp_buffer_get_ptr(cam_buf);
            size_t base_length = mpp_buffer_get_size(cam_buf);
            int fd = mpp_buffer_get_fd(cam_buf);
            ...
            double tt = what_time_is_it_now();
            std::cout << "cost   time  ------  " << tt - ttt << std::endl;
            int rets;
            ...
            // 可以换成上面的方式 一般来说 physical address > fd > virtual address
            RK_U8 *base = NULL;
            base = (RK_U8 *)mpp_buffer_get_ptr(p->frm_buf);
        ...
        ret = mpi->encode_put_frame(ctx, frame);  //输入图像进行编码


        // std::cout << "*******************" << std::endl;
        // MppBuffer buffer = mpp_frame_get_buffer(frame);
        // void *base = mpp_buffer_get_ptr(buffer);
        // src = wrapbuffer_virtualaddr(base, width, height, RK_FORMAT_YUYV_422);
        // dst = wrapbuffer_virtualaddr((void*)resize_buf, width, height, RK_FORMAT_BGR_888);
        // int ret = imcheck(src, dst, {}, {});
        // ret = imcvtcolor(src, dst, RK_FORMAT_YUYV_422, RK_FORMAT_BGR_888);
        // end = what_time_is_it_now();
        // double t_diff = end - start;
        // sum += t_diff;
        // fps = 1000.0 / t_diff;
        ...
        do {
            ret = mpi->encode_get_packet(ctx, &packet); //获得编码后的packet
            if (ret) {
